...
public class BodycamClientEvents {
    ...
    private static boolean handleHologramInteraction(Player player, net.minecraft.world.level.Level level, net.minecraft.core.BlockPos pos) {
        ...
        boolean rightClickedHologram = level.getBlockState(pos).is(Bodycam.HOLOGRAM_BLOCK.get());
        if (!rightClickedHologram) return false;

        boolean standingOnHologram = level.getBlockState(player.blockPosition().below()).is(Bodycam.HOLOGRAM_BLOCK.get()) ||
                                     level.getBlockState(player.blockPosition()).is(Bodycam.HOLOGRAM_BLOCK.get());
        if (!standingOnHologram) return false;
