...
public class PlayerSelectionScreen extends Screen {
    ...
    private static final ResourceLocation WINDOW_TEXTURE = ResourceLocation.fromNamespaceAndPath("bodycam",
            "textures/gui/gui_window.png");
    ...
    protected void init() {
        ...
        this.listWidget = new PlayerListWidget(this.minecraft, listWidth, this.height, listY, listY + listHeight, 40,
                listX);

        if (this.minecraft != null && this.minecraft.getConnection() != null) {
            for (net.minecraft.client.multiplayer.PlayerInfo info : this.minecraft.getConnection().getOnlinePlayers()) {
                if (this.minecraft.player != null
                        && !info.getProfile().getId().equals(this.minecraft.player.getUUID())) {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        drawNineSlice(graphics, WINDOW_TEXTURE, this.guiX, this.guiY, this.guiWidth, this.guiHeight, 64, 64, 512, 512,
                3840, 2160);
    ...
    private void drawNineSlice(GuiGraphics graphics, ResourceLocation texture, int x, int y, int width, int height,
            int rCW, int rCH, int uvCW, int uvCH, int texW, int texH) {
        ...
        graphics.blit(texture, x + width - rCW, y + height - rCH, rCW, rCH, texW - uvCW, texH - uvCH, uvCW, uvCH, texW,
                texH);
        graphics.blit(texture, x + rCW, y, width - 2 * rCW, rCH, uvCW, 0, texW - 2 * uvCW, uvCH, texW, texH);
        graphics.blit(texture, x + rCW, y + height - rCH, width - 2 * rCW, rCH, uvCW, texH - uvCH, texW - 2 * uvCW,
                uvCH, texW, texH);
        graphics.blit(texture, x, y + rCH, rCW, height - 2 * rCH, 0, uvCH, uvCW, texH - 2 * uvCH, texW, texH);
        graphics.blit(texture, x + width - rCW, y + rCH, rCW, height - 2 * rCH, texW - uvCW, uvCH, uvCW,
                texH - 2 * uvCH, texW, texH);
        graphics.blit(texture, x + rCW, y + rCH, width - 2 * rCW, height - 2 * rCH, uvCW, uvCH, texW - 2 * uvCW,
                texH - 2 * uvCH, texW, texH);
    }
    ...
    public class PlayerListWidget extends ObjectSelectionList<PlayerEntry> {
        ...
        public PlayerListWidget(Minecraft mc, int width, int height, int y0, int y1, int itemHeight, int listX) {
            ...
            this.listX = listX;
            this.setX(listX);
            // Methods setRenderBackground and setRenderTopAndBottom are removed/handled
            // differently in 1.21.1
        }
            ...
            this.selectButton = net.minecraft.client.gui.components.Button.builder(
                    ...
                    (btn) -> {
                        ...
                        if (!PlayerSelectionScreen.this.hasReach
                                && PlayerSelectionScreen.this.minecraft.level != null) {
                            net.minecraft.core.BlockPos targetPos = dev.ClasherHD.bodycam.client.ClientBodycamCache.positions
                                    .get(this.playerInfo.getProfile().getId());
                            String targetDim = dev.ClasherHD.bodycam.client.ClientBodycamCache.dimensions
                                    .get(this.playerInfo.getProfile().getId());
                            String myDim = Minecraft.getInstance().player.level().dimension().location().toString();

                            if (targetPos == null || targetDim == null || !myDim.equals(targetDim)
                                    || Math.sqrt(Minecraft.getInstance().player.blockPosition().distSqr(
                                            targetPos)) > (float) dev.ClasherHD.bodycam.config.ModServerConfig.MAX_MONITOR_DISTANCE
                                                    .get()) {
                                PlayerSelectionScreen.this.minecraft.player
                                        .displayClientMessage(Component.translatable("message.bodycam.signal_weak")
                        ...
                        try {
                            PlayerSelectionScreen.this.minecraft.getSoundManager()
                            ...
                            PlayerSelectionScreen.this.minecraft.setScreen(new BodycamViewScreen(
                                    this.playerInfo.getProfile().getId(), this.displayName,
                                    PlayerSelectionScreen.this.hasReach, PlayerSelectionScreen.this.isOnHologram));
                        } catch (Exception e) {
                        }
                    }).bounds(0, 0, 80, 20).build();

            if (Minecraft.getInstance().level != null) {
                int jammerMode = dev.ClasherHD.bodycam.client.ClientBodycamCache.jammers
                        .getOrDefault(this.playerInfo.getProfile().getId(), 0);
        ...
        public void render(GuiGraphics graphics, int index, int top, int left, int width, int height, int mouseX,
            ...
            if (Minecraft.getInstance().level != null) {
                ...
                if (jammerMode == 1) {
                    ...
                    String targetDim = dev.ClasherHD.bodycam.client.ClientBodycamCache.dimensions
                            .get(this.playerInfo.getProfile().getId());
                    String myDim = Minecraft.getInstance().player != null
            ...
            ResourceLocation skin = this.playerInfo.getSkin().texture();
            graphics.blit(skin, left, top + 4, 32, 32, 8.0F, 8.0F, 8, 8, 64, 64);
            graphics.blit(skin, left, top + 4, 32, 32, 40.0F, 8.0F, 8, 8, 64, 64);
            boolean hasAnonymizer = dev.ClasherHD.bodycam.client.ClientBodycamCache.anonymizers
                    .getOrDefault(this.playerInfo.getProfile().getId(), false);
            int jammerMode = dev.ClasherHD.bodycam.client.ClientBodycamCache.jammers
                    .getOrDefault(this.playerInfo.getProfile().getId(), 0);
            int nameColor;
            ...
            if (!hasAnonymizer && observingMe.contains(this.playerInfo.getProfile().getId())) {
                nameColor = 0xFF000000
                        | Integer.parseInt(dev.ClasherHD.bodycam.config.ModClientConfig.COLOR_OBSERVING.get(), 16);
            } else if (jammerMode == 1) {
                nameColor = 0xFF000000
                        | Integer.parseInt(dev.ClasherHD.bodycam.config.ModClientConfig.COLOR_BLOCKED.get(), 16);
            } else if (jammerMode == 2) {
                net.minecraft.core.BlockPos targetPos = dev.ClasherHD.bodycam.client.ClientBodycamCache.positions
                        .get(this.playerInfo.getProfile().getId());
                String targetDim = dev.ClasherHD.bodycam.client.ClientBodycamCache.dimensions
                ...
                if (!blocked) {
                    blocked = Math.sqrt(Minecraft.getInstance().player.blockPosition().distSqr(
                            targetPos)) > (double) dev.ClasherHD.bodycam.config.ModServerConfig.MAX_MONITOR_DISTANCE
                                    .get();
                }
                nameColor = blocked
                        ? (0xFF000000 | Integer
                                .parseInt(dev.ClasherHD.bodycam.config.ModClientConfig.COLOR_BLOCKED.get(), 16))
                        : (0xFF000000 | Integer
                                .parseInt(dev.ClasherHD.bodycam.config.ModClientConfig.COLOR_STANDARD.get(), 16));
            } else {
                nameColor = 0xFF000000
                        | Integer.parseInt(dev.ClasherHD.bodycam.config.ModClientConfig.COLOR_STANDARD.get(), 16);
            }
