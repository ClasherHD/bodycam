...
public class JammerItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide()) {
            ...
            if (!ModServerConfig.ENABLE_JAMMER.get()) {
                player.sendSystemMessage(Component.literal("The Jammer is disabled on this server.").withStyle(ChatFormatting.RED));
                return InteractionResultHolder.fail(stackInHand);
            }
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity entity, int slotId, boolean isSelected) {
        if (entity instanceof Player player) {
            ...
            if (!level.isClientSide() && masterStack != null) {
                long currentTime = level.getGameTime();
                if (BodycamHelper.getPersistentData(player).getLong("bodycam_jammer_last_tick") != currentTime) {
